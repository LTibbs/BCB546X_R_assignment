...
To show this, I first need to find the SNPs that vary within a group, by chromosome.

```{r}
group.snps <- genotypes %>%
  mutate_all(funs(str_replace_all(., fixed("?/?"), NA))) %>% # first recode "?/?"" as NA
  
  # look at https://stackoverflow.com/questions/45326363/using-split-to-create-new-dataframes-in-r-and-rename-based-on-variable-conditio for how to split
  
#   # for the first graph (SNPs per chromosome)
# test <- tibble(group=c("A","A","A","B","C"), chr=c(1,1,2,3,1))
# ggplot(data=test) +
#   geom_bar(mapping=aes(x=chr, fill=group))
