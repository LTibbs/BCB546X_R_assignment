...
- `BCB546X_R_assignment/src` folder contains this code and is set as the working directory (using `setwd()`)
- `BCB546X_R_assignment/data` folder already contains the input data files
- `BCB546X_R_assignment/output` folder is initially empty but will contain output files and graphs
...
Goal: plot the number of SNPs in the dataset on each chromosome, and determine what groups contribute most of these SNPs.

To do this, I want a bar chart that has bars for the count of SNPs on each chromosome, and with different fill colors corresponding to the different groups. However, to accomplish this, the data must be in "long" format, with one column for "group" and another column for "Chromosome" for each SNP. 
